...
import { parseProcessAnchorCompleteBody, processAnchorComplete } from "./handler.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const parsed = parseProcessAnchorCompleteBody(raw);
    if (!parsed.ok) {
      return json({ success: false, message: parsed.error }, 400);
    }

    const result = await processAnchorComplete(userSb, serviceSb, userData.user.id, parsed.value);
    if (!result.success) {
      const status = result.message === "Anchor not found" ? 404 : 400;
