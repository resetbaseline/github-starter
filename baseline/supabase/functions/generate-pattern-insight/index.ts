...
import { generatePatternInsight } from "./handler.ts";
import { parseGeneratePatternInsightBody } from "./parse.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const parsed = parseGeneratePatternInsightBody(raw);
    if (!parsed.ok) {
      return json({ data: null, error: { message: parsed.error, code: "validation_error" } }, 400);
    }

    const result = await generatePatternInsight(userSb, serviceSb, userId, parsed.value);
