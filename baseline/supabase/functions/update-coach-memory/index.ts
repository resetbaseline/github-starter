...
import { updateCoachMemory } from "./handler.ts";
import { parseUpdateCoachMemoryBody } from "./parse.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const msg = e instanceof Error ? e.message : String(e);
    return json({ data: null, error: { message: msg, code: "configuration_error" } }, 500);
  }
  ...
  try {
    ...
    const parsed = parseUpdateCoachMemoryBody(raw);
    if (!parsed.ok) {
      return json({ data: null, error: { message: parsed.error, code: "validation_error" } }, 400);
    }

    const result = await updateCoachMemory(userSb, serviceSb, userId, parsed.value);

    if (result.error) {
      if (result.error.code === "forbidden") {
        return json({ data: null, error: { message: result.error.message, code: "forbidden" } }, 403);
      }
      if (result.error.code === "anthropic_error") {
        let anthropic: unknown = null;
        try {
          anthropic = JSON.parse(result.error.message);
        } catch {
          /* ignore */
        }
        if (anthropic && typeof anthropic === "object" && (anthropic as { type?: string }).type === "anthropic_error") {
          return json({ data: null, error: { message: result.error.message, code: "anthropic_error" } }, 502);
        }
        return json({ data: null, error: { message: result.error.message, code: "anthropic_error" } }, 502);
      }
      if (result.error.code === "parse_error") {
        return json({ data: null, error: result.error }, 422);
      }
