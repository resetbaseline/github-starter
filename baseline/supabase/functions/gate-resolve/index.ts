...
import { gateResolve } from "./handler.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const parsed = parseGateResolveBody(raw);
    if (!parsed.ok) {
      return json({ data: null, error: { message: parsed.error, code: "validation_error" } }, 400);
    }

    const result = await gateResolve(userSb, serviceSb, userId, parsed.value);

    if (result.error) {
      const status = result.error.code === "not_found" ? 404 : result.error.code === "already_resolved" ? 409 : 400;
